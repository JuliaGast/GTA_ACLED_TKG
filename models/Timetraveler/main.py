import os
os.environ['CUDA_VISIBLE_DEVICES'] = '0' #added julia - set gpu id
import hydra 
...
#     parser.add_argument('--data_path', type=str, default='crisis2023', help='Path to data. Julia modified it so we only need dataset name here') #changed julia: Timetraveller/TITer-master/
#     parser.add_argument('--do_train',action='store_true', help='whether to train.') 
#     parser.add_argument('--do_test', default=False, action='store_true', help='whether to test.') 
#     parser.add_argument('--save_path', default='logs2', type=str, help='log and model save path.')
#     parser.add_argument('--load_model_path', default='logs2', type=str, help='trained model checkpoint path.')
...
#     # Agent Params
#     parser.add_argument('--ent_dim', default=100, type=int, help='Embedding dimension of the entities') #julia set to 80 in run.py due to A.3
...
#     parser.add_argument('--max_action_num', default=50, type=int, help='the max candidate actions number.') # Julia: this is N - A.3: 50 for ICEWS14 and ICEWS18, 60 for WIKI, and 30 for YAGO
...
#     parser.add_argument('--reward_shaping', action='store_true', help='whether to use reward shaping.') #julia in run.py: set to True
...
#     # julia added params for logging
#     parser.add_argument('--setting', type=str, default='time', choices=['time', 'static', 'raw' ]) #added julia for logging
#     parser.add_argument('--singleormultistep', type=str, default='singlestep', choices=['singlestep', 'multistep' ]) #added julia for logging
...
def main(config:DictConfig):
    ...
    model_path_name = args.data_path + '_time_singlestep_checkpoint_90.pth'
