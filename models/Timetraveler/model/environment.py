...
class Env(object):
    ...
    def next_actions(self, entites, times, query_times, max_action_num=200, first_step=False):
        ...
        actions = self.get_padd_actions(entites, times, query_times, max_action_num, first_step) #TODO I think here gt feeding single step
    ...
    def get_padd_actions(self, entites, times, query_times, max_action_num=200, first_step=False):
        ...
        for i in range(entites.shape[0]):
            ...
            if times[i] == query_times[i]: #TODO check difference between times and query_times
                action_array = self.get_state_actions_space_complete(entites[i], times[i], False)
            else:
                action_array = self.get_state_actions_space_complete(entites[i], times[i], True) #HIER WIRD GLAUBE ICH test set gefuettert! 
