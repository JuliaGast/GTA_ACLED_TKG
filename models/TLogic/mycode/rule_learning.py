...
class Rule_Learner(object):
    ...
    def save_rules(self, dt, rule_lengths, num_walks, transition_distr, seed, relsstring=''):
        """
        Save all rules.
        ...
        filename = "{0}_r{1}_n{2}_{3}_s{4}_re{5}_rules.json".format(
            dt, rule_lengths, num_walks, transition_distr, seed, relsstring
        )
    ...
    def save_rules_verbalized(
        self, dt, rule_lengths, num_walks, transition_distr, seed, relsstring=''
    ):
        """
        ...
        filename = "{0}_r{1}_n{2}_{3}_s{4}_re{5}_rules.txt".format(
            dt, rule_lengths, num_walks, transition_distr, seed, relsstring
        )
        filename = filename.replace(" ", "")
        with open(self.output_dir + filename, "w", encoding="utf-8") as fout:
