...
def get_window_edges(all_data, test_query_ts, learn_edges, window=-1, first_test_query_ts=0): #modified julia: added first_test_query_ts
    """
    ...
    If window is -2, all edges from train and validation set are used. modified by julia.
    ...
    Parameters:
        ...
        first_test_query_ts (int): smallest timestamp from test set
    ...
    if window > 0:
        mask = (all_data[:, 3] < test_query_ts) * (
            all_data[:, 3] >= test_query_ts - window #schaut das in die zukunft oder was? 
        )
    ...
    elif window == -2: #modified julia: added this option
        ...
        mask = (all_data[:, 3] < first_test_query_ts) * (
            all_data[:, 3] >= first_test_query_ts - abswindow #schaut das in die zukunft oder was? 
        )
        window_edges = store_edges(all_data[mask])
    return window_edges
