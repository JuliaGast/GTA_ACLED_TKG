...
def sort_and_rank(score, target):
    ...
    indices = torch.zeros(target.shape).to(score.device)
    for index in range(0, len(score)):
        _, sorted = torch.sort(score[index], descending=True)
        indices[index] = torch.nonzero(sorted == target[index])[0]
...
def stat_ranks(rank_list, method, log):
    hits = [1, 3, 10]
...
def load_data(dataset, bfs_level=3, relabel=False):
    ...
    elif dataset in ['ICEWS18', 'ICEWS14', "GDELT", "SMALL", "ICEWS14", "ICEWS05-15","YAGO",
                     "WIKI", 'crisis2022', 'crisis2023', 'crisis2023_gta']:
...
def construct_snap(test_triples, num_nodes, num_rels, final_score, topK):
    ...
    for _ in range(len(test_triples)):
        for index in top_indices[_]:
            ...
            if r < num_rels:
                predict_triples.append([test_triples[_][0].item(), r.item(), index.item()]) # MODIFIED JULIA
                # predict_triples.append([test_triples[_][0], r, index]) #original version
            else:
                predict_triples.append([index.item(), r.item()-num_rels, test_triples[_][0].item()]) # MODIFIED JULIA
                # predict_triples.append([index, r-num_rels, test_triples[_][0]])  #original version

    # 转化为numpy array
    predict_triples = np.array(predict_triples, dtype=int) # MODIFIED JULIA
