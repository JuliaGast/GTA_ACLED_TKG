...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, use_cuda):
        ...
        if self.use_static:
            ...
            self.h = static_emb #TODO: do they init the h with the static embedding?! as we wanted to?! schad.
        else:
        ...
        for i, g in enumerate(g_list):
            g = g.to(self.gpu)
            temp_e = self.h[g.r_to_e] #what's r_to_e???
            ...
            for span, r_idx in zip(g.r_len, g.uniq_r):
                ...
                x_mean = torch.mean(x, dim=0, keepdim=True) # hier evtl aggregation von embeddings fuer die ral bas auf sub/ob
                x_input[r_idx] = x_mean
            if i == 0:
                x_input = torch.cat((self.emb_rel, x_input), dim=1)
                self.h_0 = self.relation_cell_1(x_input, self.emb_rel)    # 第1层输入 layer1 input #GRU
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
            else:
                x_input = torch.cat((self.emb_rel, x_input), dim=1)
                self.h_0 = self.relation_cell_1(x_input, self.h_0)  # 第2层输出==下一时刻第一层输入 Layer 2 output == first layer input at the next moment
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
            current_h = self.rgcn.forward(g, self.h, [self.h_0, self.h_0]) #WICHTIG here is the rgcn forward - TODO CHECK IT
            current_h = F.normalize(current_h) if self.layer_norm else current_h
            time_weight = F.sigmoid(torch.mm(self.h, self.time_gate_weight) + self.time_gate_bias)  # time gate (4) in paper
            self.h = time_weight * current_h + (1-time_weight) * self.h                             # (5) in paper
            history_embs.append(self.h)
        return history_embs, static_emb, self.h_0, gate_list, degree_list


    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda, subbatch=0):
        with torch.no_grad():
            ...
            score = torch.zeros((len(all_triples), self.num_ents), dtype=torch.float, device=all_triples.device)
            score_rel = torch.zeros((len(all_triples), 2*self.num_rels), dtype=torch.float, device=all_triples.device)
            if subbatch > 0:
                for i in range(0, len(all_triples), subbatch):
                    score[i:i+subbatch] = self.decoder_ob.forward(embedding, r_emb, all_triples[i:i+subbatch,:], mode="test")
                    score_rel[i:i+subbatch] = self.rdecoder.forward(embedding, r_emb, all_triples[i:i+subbatch,:], mode="test")                   
            else:
    ...
    def get_loss(self, glist, triples, static_graph, use_cuda): #triples: I think this is the target. glist: listory of dgl graphs, len e.g. 6
        """
        :param glist:
        :param triplets:
        :param static_graph: 
